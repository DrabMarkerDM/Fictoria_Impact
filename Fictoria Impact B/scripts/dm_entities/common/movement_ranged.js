               ...
               // ===== 核心控制：水域速度精准分流分级（深水 0.2 / 浅水 0.5） =====
                ...
                if (isFloatingInWater) {
                    ...
                    if (dy > 0.5 && !isAtWaterSurface) {
                        jumpImpulse = 0.06; // 目标在上方且未到水面，平滑上浮
                    } else if (dy < -0.5) {
                        jumpImpulse = -0.10; // 目标在下方，平滑下潜
                    } else {
                        jumpImpulse = isAtWaterSurface ? -0.02 : 0.01; // 在水面或高度持平时对抗原版浮力
                    }
                } else {
                ...
                // 最终解算写入物理冲量（液体下 1/3 速度限制已无缝作用于 finalSpeed）
